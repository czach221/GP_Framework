...
class Fitness():
    ...
    @staticmethod
    def calculate_mse(x_data, y_data, func):
        """
        ...
        # Transform x_data into an appropriate shape
        x_data = np.array(x_data)
        t = ([x_data[:, i] for i in range(x_data.shape[1])])
    ...
    def _calculate_fitness(x_data, y_data, y_pred):
        y_mean = np.mean(y_data)
    ...
    @staticmethod
    def calculate_r2_2(x_data, y_data, func):
        x_symbs = [symbols(f'x_{i}') for i in range(len(x_data[0]))]
        exec_func = lambdify(x_symbs, func, modules="numpy")
        x_data = np.array(x_data)
        y_pred = exec_func(*[x_data[:, i] for i in range(x_data.shape[1])])
        ...
        if not np.all(np.isfinite(y_pred)): 
            return float(-1e7)
        return Fitness._calculate_fitness(x_data, y_data, y_pred)
